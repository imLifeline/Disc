...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        let itemList = await db.set(`shop`, itemsData);
        if (!itemList) {
            itemList = [];
        ...
        if(!options) {
            ...
            if(userBalance < itemData.price * amount) {
                    ...
                    message.reply({ content: `You have enough coins in your bank, would you like to withdraw and buy the item?\nType: \`yes\` if you wish to do so!`, allowedMentions: { repliedUser: false } }).then(async msg => {
                        ...
                        collector.on('collect', async m => {
                            if(m.content.toLowerCase() === 'yes') {
                                await db.subtract(`user_${message.author.id}.bank`, itemData.price * amount);
                                let userInventory = await db.get(`user_${message.author.id}.inventory`);
                                if(!userInventory) {
                                    userInventory = [];
                                    await db.set(`user_${message.author.id}.inventory`, userInventory);
                                }
                ...
                await db.set(`user_${message.author.id}.balance`, userBalance - itemData.price * amount);
            ...
            let userItem = userInventory.find(obj => obj.item.value === itemData.value);
            if(!userItem) return message.reply({ content: `${client.config.deny} | You don't have that item!`, allowedMentions: { repliedUser: false } });
            if(userItem.quantity < amount) return message.reply({ content: `${client.config.deny} | You don't have that much of that item!`, allowedMentions: { repliedUser: false } });
            let userBalance = await db.get(`user_${message.author.id}.balance`);
            let sellAmount = Math.floor((itemData.price * amount) * 0.75);
            await db.set(`user_${message.author.id}.balance`, userBalance + sellAmount);
            userItem.quantity -= amount;
            await db.set(`user_${message.author.id}.inventory`, userInventory);
            message.reply({ embeds: [embed.Embed_shop_sell(itemData, sellAmount)], allowedMentions: { repliedUser: false } });
        } else if(options.toLowerCase() === 'info') {
            message.reply({ embeds: [embed.Embed_shop(shopDesc)], allowedMentions: { repliedUser: false } });
        } else {
            message.reply({ embeds: [embed.Embed_shop(shopDesc)], allowedMentions: { repliedUser: false } });
        }
